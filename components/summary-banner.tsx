...
export function SummaryBanner({
  ...
  const body =
    status === 'failed' ? (
      ...
      <p className="flex items-start gap-2 text-sm text-bark">
      ...
      <p className="text-sm leading-relaxed text-cocoa">{summary}</p>
    ) : (
      <p className="text-sm text-bark/80">Generating summary…</p>
    );

  return (
    <div className="shrink-0 border-b bg-cream px-4 py-3 sm:px-6">
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h1 className="min-w-0 truncate text-lg font-semibold tracking-tight">{filename}</h1>
        {pageCount ? (
          <span className="text-xs text-bark/80">{pageCount} pages</span>
        ) : null}
      </div>

      {/* Collapsed on small screens so the PDF itself stays the focus. */}
      <details className="mt-2 lg:hidden" open>
        <summary className="cursor-pointer text-xs font-medium uppercase tracking-wide text-bark/80">
          AI summary
        </summary>
      ...
      <div className="mt-2 hidden items-start gap-2 lg:flex">
        <Sparkles className="mt-0.5 size-4 shrink-0 text-ember" aria-hidden />
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-bark/80">AI summary</p>
